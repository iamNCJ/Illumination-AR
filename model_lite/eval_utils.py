...
def calIntsAcc(gt_i, pred_i, data_batch=1):
    ...
    gt_i  = gt_i / ref_int
    scale = torch.linalg.lstsq(gt_i.view(-1, 1), pred_i.view(-1, 1))
...
def SphericalDirsToClass(dirs, cls_num: int):
...
def SphericalClassToDirs(x_cls, y_cls, cls_num: int):
    # print(x_cls.shape)
...
def LightIntsToClass(ints, cls_num: int):
    ints = (ints - 0.2) / 1.8
    ints = (ints * cls_num).clamp(0, cls_num-1).long()
    return ints.view(-1)

def ClassToLightInts(cls, cls_num: int):
